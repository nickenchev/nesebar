...
void Core<Memory, Mapping, DecimalMode>::step()
{
	...
	switch (opcode)
	{
		...
		case SRE::ZeroPage::value:
		{
			perform<SRE::ZeroPage>([this](const MemAccess access) { sre(access); });
			break;
		}
		case SRE::ZeroPageX::value:
		{
			perform<SRE::ZeroPageX>([this](const MemAccess access) { sre(access); });
			break;
		}
		case SRE::Absolute::value:
		{
			perform<SRE::Absolute>([this](const MemAccess access) { sre(access); });
			break;
		}
		case SRE::AbsoluteX::value:
		{
			perform<SRE::AbsoluteX>([this](const MemAccess access) { sre(access); });
			break;
		}
		case SRE::AbsoluteY::value:
		{
			perform<SRE::AbsoluteY>([this](const MemAccess access) { sre(access); });
			break;
		}
		case SRE::IndexedIndirect::value:
		{
			perform<SRE::IndexedIndirect>([this](const MemAccess access) { sre(access); });
			break;
		}
		case SRE::IndirectIndexed::value:
		{
			perform<SRE::IndirectIndexed>([this](const MemAccess access) { sre(access); });
			break;
		}
		case RRA::ZeroPage::value:
		{
			perform<RRA::ZeroPage>([this](const MemAccess access) { rra(access); });
			break;
		}
		case RRA::ZeroPageX::value:
		{
			perform<RRA::ZeroPageX>([this](const MemAccess access) { rra(access); });
			break;
		}
		case RRA::Absolute::value:
		{
			perform<RRA::Absolute>([this](const MemAccess access) { rra(access); });
			break;
		}
		case RRA::AbsoluteX::value:
		{
			perform<RRA::AbsoluteX>([this](const MemAccess access) { rra(access); });
			break;
		}
		case RRA::AbsoluteY::value:
		{
			perform<RRA::AbsoluteY>([this](const MemAccess access) { rra(access); });
			break;
		}
		case RRA::IndexedIndirect::value:
		{
			perform<RRA::IndexedIndirect>([this](const MemAccess access) { rra(access); });
			break;
		}
